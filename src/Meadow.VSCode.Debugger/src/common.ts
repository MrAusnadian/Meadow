...
export function validateDotnetVersion() {
    ...
    try {
        ...
        if (dotnetVersionResult.error) {
            let errObj = <any>dotnetVersionResult.error;
            if (errObj.code === 'ENOENT') {
                errorMessage = `dotnet is not installed or could not be found in PATH.`
            }
            else {
                errorMessage = dotnetVersionResult.error.message + `. Error identifying dotnet SDK version; ensure it is properly installed.`;
            }
        }
        else if (dotnetVersionResult.status !== 0) {
            let dotnetError = dotnetVersionResult.stdout.toString() + dotnetVersionResult.stderr.toString();
            errorMessage = `dotnet returned exit code ${dotnetVersionResult.status} - ` + dotnetError;
        }
        else {
            let dotnetVersionString = semver.coerce(dotnetVersionResult.stdout.toString().trim());
